...
## Roadmap
- [ ] Full compatibility with existing torch modules. This is something the competition does *not* have, because they all made the mistake of going for shape `(..., channels_in, blades)` where blades is $2^n$. In `kingdon` v3 however, a multivector has shape `(..., channels_in)` and the blade dimension is invisible. (Under the hood the underlying datastructure is `(blades, ..., channels_in)` where moreover blades does not have to be the full algebra, which is where we will get our speed-up from.) The torch modules expect the channels to be the last dimension, which kingdon has!
